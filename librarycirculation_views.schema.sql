...
-- Week 5 — Views
-- Create reusable virtual tables for simplified reporting


CREATE VIEW IF NOT EXISTS v_patron_activity AS
SELECT 
    p.c1 AS patron_id,
    p.c3 || ' ' || p.c4 AS patron_name,
    p.c7 AS status,
    COUNT(DISTINCT l.c1) AS total_loans,
    SUM(CASE WHEN f.c4 = 0 THEN f.c3 ELSE 0 END) AS unpaid_fines
FROM patrons_1 p
LEFT JOIN loans_1 l ON p.c1 = l.c3
LEFT JOIN fines_1 f ON l.c1 = f.c2
GROUP BY p.c1, p.c3, p.c4, p.c7;

CREATE VIEW IF NOT EXISTS v_branch_performance AS
SELECT 
    b.c1 AS branch_id,
    b.c2 AS branch_name,
    COUNT(DISTINCT c.c1) AS total_copies,
    COUNT(DISTINCT l.c1) AS total_loans,
    SUM(f.c3) AS total_fines
FROM branches_1 b
LEFT JOIN copies_1 c ON b.c1 = c.c3
LEFT JOIN loans_1 l ON b.c1 = l.c4
LEFT JOIN fines_1 f ON l.c1 = f.c2
GROUP BY b.c1, b.c2;

CREATE VIEW IF NOT EXISTS v_catalog_status AS
SELECT 
    b.c1 AS book_id,
    b.c3 AS title,
    COUNT(c.c1) AS total_copies,
    SUM(CASE WHEN l.c7 IS NULL THEN 1 ELSE 0 END) AS checked_out,
    SUM(CASE WHEN l.c7 IS NOT NULL THEN 1 ELSE 0 END) AS returned
FROM books_1 b
LEFT JOIN copies_1 c ON b.c1 = c.c2
LEFT JOIN loans_1 l ON b.c1 = l.c2
GROUP BY b.c1, b.c3;
